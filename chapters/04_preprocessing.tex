...
Die Vorverarbeitungsschritte der \ac{LiDAR}-Punktwolken werden in diesem Kapitel gebündelt. Ziel ist es, Datenvolumen und Rauschen vor den nachfolgenden Modulen zu reduzieren und gleichzeitig eine gleichmäßig verteilte Datenbasis für Segmentierung und Clustering zu schaffen. Im Zentrum stehen die räumliche Begrenzung per CropBox und die Ausdünnung mittels VoxelGrid, jeweils mit den verwendeten Parametern und der Umsetzung in der \ac{ROS}~2-Verarbeitungskette.
...
Neben den Pflichtfeldern \(x,y,z\) unterstützt der Node optionale Felder (\texttt{intensity}, \texttt{ring}) und erlaubt über die Callback-API \texttt{add\_on\_set\_parameters\_callback} eine Laufzeitänderung der Grenzen ohne Neustart des Nodes. Die Startparameter sind in Tabelle~\ref{tab:cropbox_params} zusammengefasst.

\begin{table}[H]
  \centering
  \begin{tabular}{lc}
    \toprule
    \textbf{Parametername} & \textbf{Startwert} \\ \midrule
    \texttt{input\_topic}  & \texttt{/ouster/points} \\
    \texttt{output\_topic} & \texttt{/points\_cropped} \\
    \texttt{min\_bound}    & \([-20.0, -15.0, -3.0]\) \\
    \texttt{max\_bound}    & \([20.0, 15.0, 5.0]\) \\ \bottomrule
  \end{tabular}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/cropbox_compare.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.82\textwidth]{images/voxelgrid_prinzip.png}%
...
\begin{table}[H]
  ...
  \begin{tabular}{lc}
    \toprule
    \textbf{Parametername} & \textbf{Startwert} \\ \midrule
    \texttt{input\_topic}  & \texttt{points\_cropped} \\
    \texttt{output\_topic} & \texttt{points\_voxel} \\
    \texttt{voxel\_size}   & \SI{0,20}{\meter} \\ \bottomrule
  \end{tabular}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/voxel_compare.png}
