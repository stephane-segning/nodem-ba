...
Topic \texttt{/ouster/points} \\[0.15cm]
\midrule

2 &
Objektliste mit Pose und Abmessungen &
Track-IDs konsistent über $\geq 5$ Frames \\[0.15cm]
\midrule

3 &
Bodenpunkte und Störsignale entfernen &
\textbf{Minimierung des Restbodenanteils} \\[0.15cm]
\midrule

4 &
Reduzierte Punktdichte ohne wesentlichen Geometrieverlust &
Punktdichte $\geq70\,\%$ im $3\,\text{m}$-Gitter; \textbf{Hohe geometrische Genauigkeit} \\[0.15cm]
\midrule
...
Darstellung von Rohdaten, Bodenfilter, Clustern und Tracks in RViz2/GUI \\[0.05cm]
\bottomrule

\end{tabularx}
\caption{Anforderungen an die Verarbeitungskette mit zugehörigen Messkriterien}
\label{tab:anforderungen_verarbeitungskette}
\end{table}
...
Die Datenerfassung umfasst sämtliche Schritte von der Initialisierung des Ouster OS1 über die Netzwerkkommunikation bis hin zur Bereitstellung der Punktwolken innerhalb des \ac{ROS}~2-Systems. Der grundlegende Aufbau orientiert sich an der Messkette nach \cite{Wendel2025}. 
...
Abbildung~\ref{fig:messkette_sensordaten} zeigt den schematischen Aufbau der Datenerfassungskette mit dem Ouster OS1-\ac{LiDAR}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{images/messkette_sensordaten.png}
  \caption{Aufbau der Datenerfassung mit dem Ouster OS1-\ac{LiDAR} \cite{Wendel2025}).}
...
crop\_box\_node & Sub & \texttt{/ouster/points} & PointCloud2       & SensorDataQoS (Best Effort) \\
crop\_box\_node & Pub & \texttt{/points\_cropped} & PointCloud2     & SensorDataQoS (Best Effort) \\
\midrule

voxel\_filter\_node & Sub & \texttt{/points\_cropped} & PointCloud2    & SensorDataQoS (Best Effort) \\
voxel\_filter\_node & Pub & \texttt{/points\_voxel} & PointCloud2      & SensorDataQoS (Reliable) \\
\midrule

ransac\_ground\_node & Sub & \texttt{/points\_voxel} & PointCloud2      & SensorDataQoS (Best Effort) \\
ransac\_ground\_node & Pub & \texttt{/obstacle\_points} & PointCloud2   & rclcpp::\ac{QoS}(10) (Reliable) \\
\midrule

cluster\_extraction\_node & Sub & \texttt{/obstacle\_points} & PointCloud2  & SensorDataQoS (Best Effort) \\
cluster\_extraction\_node & Pub & \texttt{/detections\_markers} & MarkerArray & rclcpp::\ac{QoS}(10) (Reliable) \\
cluster\_extraction\_node & Pub & \texttt{/detections\_raw} & Detection3DArray & SensorDataQoS (Best Effort) \\
\midrule

sort\_tracker\_node & Sub & \texttt{/detections\_raw} & Detection3DArray & SensorDataQoS (Best Effort) \\
sort\_tracker\_node & Pub & \texttt{/tracks\_raw} & Detection3DArray  & rclcpp::\ac{QoS}(10) (Reliable) \\
sort\_tracker\_node & Pub & \texttt{/tracks\_markers} & MarkerArray     & rclcpp::\ac{QoS}(10) (Reliable) \\
\bottomrule
...
\begin{table}[H]
  ...
  \begin{tabular}{p{2.2cm}p{13cm}}
    \toprule
    \textbf{Feld} & \textbf{Bedeutung} \\ \midrule

    \texttt{x, y, z} &
    Kartesische Koordinaten (m) im \texttt{frame\_id}.
    Rechtsdrehendes System nach REP~103: \(x\) vorwärts, \(y\) links, \(z\) oben. \\ \midrule

    \texttt{intensity} &
    Reflektierte Signalstärke (\texttt{uint16}); abhängig von Sensorkalibrierung und daher nur innerhalb eines Scans vergleichbar. \\ \midrule

    \texttt{ring} &
    Vertikaler Kanalindex (Laser/Detektor). Beim OS1-64: 64 Ringe von 0 (unten) bis 63 (oben). \\ \midrule

    \texttt{time} &
    Zeitversatz eines Punktes relativ zu \texttt{header.stamp} (s); wichtig für Motion Compensation innerhalb eines Scans. \\ \bottomrule
...
Tabelle~\ref{tab:anforderungen_verarbeitungskette} formulierten Messkriterien: \textbf{Anforderung~6}
...
Die vorgestellte Systemarchitektur definiert eine modulare und echtzeitfähige Verarbeitungskette, die auf etablierten ROS~2-Standards und PCL-Algorithmen basiert. Durch die klare Trennung in funktionale Knoten, die Verwendung standardisierter Nachrichtenformate und die Definition messbarer Qualitätskriterien ist das System sowohl wartbar als auch objektiv bewertbar.

Der erste Schritt dieser Kette, die Vorverarbeitung der Punktwolken, wird im anschließenden Kapitel detailliert beschrieben und implementiert.