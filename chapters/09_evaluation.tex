...
Eine effektive Bodenentfernung wird dabei durch die geringe Anzahl an Falsch-Positiv-Detektionen (False Positives) in den folgenden Szenarien demonstriert, auch ohne direkte prozentuale Messung. Die geometrische Genauigkeit der Bounding Boxes wurde dabei visuell verifiziert und für die Anforderungen der Objektverfolgung als ausreichend bewertet.
...
Tabelle~\ref{tab:messaufzeichnungen} gibt einen Überblick über die verwendeten Messaufzeichnungen und deren Eigenschaften.

\begin{table}[H]
  \centering
  \caption{Übersicht der verwendeten Messaufzeichnungen}
  \label{tab:messaufzeichnungen}
  \begin{tabular}{clll}
    \toprule
    \textbf{Szenario} & \textbf{Beschreibung} & \textbf{Messaufzeichnung} & \textbf{Zeitraffer} \\ \midrule
    S1 & Geparkte Autos & 2025-10-15-VK-OL-006 & Start: 5s; run 10s \\
    S2 & Kreuzung & 2025-10-15-VK-OL-007 & Start: 20s; run 10s \\
    S3 & Unebene Fläche & 2025-10-15-VK-OL-004 & Start: 145s; run 10s \\ \bottomrule
  \end{tabular}
\end{table}
...
Für das Szenario der geparkten Fahrzeuge wurden zunächst die Auswirkungen der Voxelgröße untersucht. Tabelle~\ref{tab:geparkt_voxel} zeigt die Ergebnisse.
...
Anschließend wurde der Einfluss des Schwellenwerts für die Bodensegmentierung analysiert, wie in Tabelle~\ref{tab:geparkt_distance} dargestellt.
...
Schließlich wurde die Cluster-Toleranz variiert. Die Ergebnisse sind in Tabelle~\ref{tab:geparkt_cluster_tolerance} aufgeführt.
...
Die geometrische Genauigkeit der Bounding Boxes wurde dabei visuell verifiziert und für die Anforderungen der Objektverfolgung als ausreichend bewertet.

\subsection{S2 – Kreuzung}

Die Parameterstudie für die Kreuzungsszene untersucht die Auswirkungen verschiedener Voxelgrößen. Tabelle~\ref{tab:kreuzung_voxel_size} listet die Anzahl der Detektionen pro Klasse auf.
...
Der Einfluss des distance\_threshold-Parameters auf die Detektionsleistung in der Kreuzungsszene wird in Tabelle~\ref{tab:kreuzung_distance_threshold} dargestellt.
...
Die cluster\_tolerance-Werte und ihre Auswirkungen auf die Objekterkennung in der Kreuzungsszene sind in Tabelle~\ref{tab:kreuzung_cluster_tolerance} zusammengefasst.
...
Auch für die unebene Fahrbahn wurde der Einfluss der Voxelgröße untersucht. Tabelle~\ref{tab:uneben_voxel} zeigt die Detektionsergebnisse.

\begin{table}[H]
\centering
\caption{Anzahl Detektionen pro Klasse für verschiedene voxel\_size (Unebene Fläche).}
\label{tab:uneben_voxel}
\begin{tabular}{cccccc}
\toprule
\textbf{voxel\_size} & \textbf{PKW} & \textbf{Mensch} & \textbf{Fahrradfahrer} & \textbf{LKW} & \textbf{Gesamt} \\
\midrule
...
Tabelle~\ref{tab:uneben_distance} stellt die Ergebnisse für verschiedene Schwellenwerte der Bodensegmentierung auf unebener Fahrbahn dar.

\begin{table}[H]
\centering
\caption{Anzahl Detektionen pro Klasse für verschiedene distance\_threshold (Unebene Fläche).}
\label{tab:uneben_distance}
\begin{tabular}{cccccc}
\toprule
\textbf{distance\_threshold} & \textbf{PKW} & \textbf{Mensch} & \textbf{Fahrradfahrer} & \textbf{LKW} & \textbf{Gesamt} \\
\midrule
...
Abschließend zeigt Tabelle~\ref{tab:uneben_cluster} die Auswirkungen der Cluster-Toleranz in diesem Szenario.

\begin{table}[H]
\centering
\caption{Anzahl Detektionen pro Klasse für verschiedene cluster\_tolerance (Unebene Fläche).}
\label{tab:uneben_cluster}
\begin{tabular}{cccccc}
\toprule
\textbf{cluster\_tolerance} & \textbf{PKW} & \textbf{Mensch} & \textbf{Fahrradfahrer} & \textbf{LKW} & \textbf{Gesamt} \\
\midrule
...
Verarbeitungskette. Da die gemessene Latenz durchgehend unter \(100\,\text{ms}\) liegt, deutet dies darauf hin, dass der Algorithmus in einem Live-Setup ohne I/O-Limitierung höhere Raten erreichen kann.
...
Verarbeitungskette zurückzuführen.
...
Abbildung~\ref{fig:gerade_strecke_stand} zeigt das Szenario \enquote{Gerade Strecke, Stand} mit der Frontkameraansicht und der entsprechenden LiDAR-Punktwolke.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{images/gerade_strecke_stand.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/kurve_szene.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/steigung_szene.png}
...
Eine Ausgabe von \texttt{resource\_probe.py} konnte wegen eines vorzeitig beendeten Prozesses nicht gespeichert werden.
...
Abbildung~\ref{fig:urban_unebener_boden} zeigt die Robustheit der Verarbeitungskette bei realen Straßenprofilen und variierender Bodenhöhe. Das urbane Szenario mit unebenem Boden umfasst eine städtische Straße mit leicht unregelmäßiger Fahrbahnoberfläche, wobei die LiDAR-Visualisierung in RViz detektierte PKW-Clustern anzeigt.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/urban_unebener_boden.png}
    \caption{Urbanes Szenario mit unebenem Boden: Fisheye-Kameraansicht und LiDAR-Visualisierung (2025-11-21-Messdaten/2025-11-21-VK-OL-007).}
